...
#This reads in the file and fixes the format for calculations.
def file_read(): 
    try: 
        with open("a7_dataset.txt", "r") as data_file:
...
#Main function to call in all the above for processing. 
def __main__():  
    file_read() 
    try:
    ...
    Mean:  {round(mean,2)} \nMedian:  {round(median,2)} \nMode:  {round(mode, 2)} ")
        ...
        std_dev1(dataset, mean)
    except ZeroDivisionError:
        print("Cannot calculate. Nothing was read in")
    
    try:
        #Setting the manual standard deviation function as a callable variable. 
        std_dev = std_dev1(dataset, mean)
...
{round(emp_percent3,1)}% was 3 standard deviations. \n")
        ...
        plt.title("Plot 2- Standard Deviation Plot", fontsize = "x-large")
        
    #EXTRA CREDIT:
        #This creates buckets for Pandas to sort the data into, calculate total, and display. 
        df = pd.read_csv('a7_dataset.csv')
    ...
    20:25 = {len(bucket6)}")
        ...
        print("Cannot graph or finish calculations, nothing read in.")
__main__()