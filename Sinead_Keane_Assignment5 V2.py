...
#this initializes the class.    
    def __init__(self, polygon_input, sides_input, length_input):
        self.sides_input = sides_input
        self.length_input = length_input
        self.polygon_input = polygon_input.capitalize() 

#this adds the shape to the shape_list for counting purposes.   
    def shape_name(self,polygon_input):
        shape = polygon_input
...
#this does all the area calulations when called for specific numbers        
    def polygon_area(self, sides_input,length_input):
        if sides_input == 3:
            h = (.5)*math.sqrt(3)*length_input
            tri_area = round(0.5*length_input*h)
            return tri_area
        elif sides_input == 4:
            sqr_area = length_input**2
            return sqr_area
        elif sides_input == 5:
            pent_area = round(.25*math.sqrt(5*(5+2*math.sqrt(5)))*length_input**2)
            return pent_area
        elif sides_input == 6:
            hex_area = (3*math.isqrt(3))/2 * (length_input**2)
            return hex_area
#this does the perimeter when called.
    def polygon_perim(self, sides_input, length_input):
        perim = sides_input*length_input
        return perim 
